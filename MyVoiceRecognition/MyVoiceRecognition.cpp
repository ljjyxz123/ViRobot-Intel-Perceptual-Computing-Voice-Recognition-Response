...
MyVoiceRecognition::MyVoiceRecognition()//PXCSession *session
{
	...
	g_source->SetVolume(0.1f);

	//Time Related Stuffs
	tLastTime = time(NULL);
	gmtLastTime = gmtime(&tLastTime);
}
...
void PXCAPI MyVoiceRecognition::OnRecognition(const PXCSpeechRecognition::RecognitionData *data)
{
	...
	else 
	{
		...
		std::string response = voiceCmds.cmds[label].response;
		printf("Voice recognized: speach [%s] command [%s]\n", speech.c_str(), command.c_str());

		
		//时间小于2s接收到的命令不处理
		tCurrTime = time(NULL);
		gmtCurrTime = gmtime(&tCurrTime);
		printf("Current Time is : %d\n", gmtCurrTime->tm_sec);
		if((abs(gmtCurrTime->tm_sec - gmtLastTime->tm_sec) < 2) && (command == commandLastTime))
		{
			tLastTime = time(NULL);
			gmtLastTime = gmtime(&tCurrTime);
			return ;
		}
		/////////////////////////////////
		commandLastTime = command;

		printf("%d\n", data->scores[0].label);
		//speaker.speak(L"Voice recognition system start...");
		speaker.speak(response);
		printf("data->scores[0].label: %d\n", data->scores[0].label);
		/*if("turn right" == command)
		{
			printf("hello brad\n");
			speaker.speak(response);
		}*/
...
int _tmain(int argc, char** argv)
{
	...
	//语音合成系统
	//MyVoiceSynthesis speaker;
	myVoiceRecog.speaker.speak(L"Voice recognition system start...");
	//test();
